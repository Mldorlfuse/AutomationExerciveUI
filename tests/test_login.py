...
def test_full_registration_path(open_login_page, header_component, login_page, random_user_data):
...
def test_authorization_using_non_existent_data(open_login_page, login_page, random_wrong_user_data):
    login_page.login_with_incorrect_data(random_wrong_user_data['error_email'], random_wrong_user_data['error_password'])

@allure.epic("UI")
@allure.feature("Авторизация/регистрация")
@allure.story("Авторизация с существующими данными из списка тестовых пользователей")
def test_authorization_test_data_and_logout(open_login_page, header_component, login_page):
...
def test_full_registration_path_with_delete(open_login_page, header_component, login_page, random_user_data):
    login_page.signup_fill_form(random_user_data)
    login_page.enter_account_information(random_user_data)
    login_page.check_user_name(random_user_data['username'],header_component.get_user_name())
...
def test_authorization_with_already_exist_email(open_login_page, login_page):
